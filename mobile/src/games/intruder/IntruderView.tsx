...
export function IntruderView({ state }: { state: IState }) {
  ...
  const optStyle = (i: number) => {
    ...
    if (i === state.myAnswer) return { backgroundColor: '#D35400', borderColor: '#D35400' };
    return { backgroundColor: theme.colors.surface, borderColor: '#D35400' };
  };
  ...
  return (
    <View style={styles.root}>
      <View style={styles.metaRow}>
        ...
        <Pill label={`${state.answeredCount}/${state.totalPlayers}`} color="#D35400" />
      </View>
      {!reveal && <View style={styles.barBg}><View style={[styles.bar, { width: `${ratio * 100}%` }]} /></View>}

      <Title style={styles.title}>Quel est l’intrus ?</Title>
      {reveal && <Body style={styles.hint}>(catégorie : {state.hint})</Body>}
      {reveal && (
        <Body style={[styles.verdict, { color: state.iWasRight ? theme.colors.success : theme.colors.danger }]}>
          {state.myAnswer == null ? '⏱️ Trop tard !' : state.iWasRight ? '✓ Bien vu !' : '✗ Raté !'}
        </Body>
      )}

      <View style={styles.grid}>
        {state.items.map((opt, i) => (
...
const styles = StyleSheet.create({
  ...
  bar: { height: 8, borderRadius: 4, backgroundColor: '#D35400' },
  title: { textAlign: 'center', fontSize: theme.font.h2 },
  hint: { textAlign: 'center', color: theme.colors.textMuted },
  ...
  option: { width: '48%', minHeight: 70, borderRadius: theme.radius.md, borderWidth: 2, alignItems: 'center', justifyContent: 'center', padding: theme.spacing(1) },
  ...
  score: { color: '#D35400', fontWeight: '800' },
});
